    ...
    while float(val) <= 0.95:
        ...
        try:
            ...
            if tmp_rate is None:
                return 0
            val = str(tmp_rate)
            print("视频" + video_id + " " + video_name + " 学习进度为：\t" + str(float(val) * 100) + "%/100%")
            time.sleep(2)
        except Exception as e:
            print(e.__str__())
            pass
        ...
        if not (number.isdigit()) or int(number) > len(your_courses):
            ...
            for ins in your_courses:
                ...
                for one_video in homework_dic.items():
                    # 创建线程并启动
                    t = threading.Thread(target=one_video_watcher, args=(
                    one_video[0], one_video[1], ins["course_id"], user_id, ins["classroom_id"]))
                    t.start()
                    threads.append(t)
                # 等待所有线程执行完毕
            for t in threads:
